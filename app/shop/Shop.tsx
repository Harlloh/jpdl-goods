    ...
    return (
        ...
        <div className="sticky top-0 left-0 right-0 items-center flex justify-between p-2 z-10 bg-teal-600 w-max-100 overflow-x-auto">
          {categories.map((category: any) => (
            <span key={category.name} className="mr-4 text-white cursor-pointer" onClick={() => handleScrollToCategory(category.name)}>
              {category.name}
            </span>
          ))}
        </div>
      <Container>
        <div className="w-100 items-center flex flex-col relative mt-4">
          <h1 className="text-2xl text-black font-semibold my-5">Welcome to our store</h1>
          <span className="sticky w-75 p-2 border border-gray-300 rounded focus:outline-none focus:border-[0.5px] focus:border-teal-500 w-80 flex items-center justify-between">
            ...
            />
            {searchTerm && <FaTimes onClick={handleClearSearchTerm} className="cursor-pointer" />}
          </span>
          {Object.keys(searchedProducts).map((category) => (
            <div key={category} className="my-9 mx-auto w-100" id={`${sanitizeForId(category)}`}>
              <h2 className="text-2xl font-semibold text-black mb-6">{category}</h2>
